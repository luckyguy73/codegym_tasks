...
public class DrugController {
    ...
    public synchronized void buy(Drug drug, int count) {
        ...
        if (currentCount < count) {
            System.out.printf("%s wants %d tablets of %s. In stock: %d%n", name, count, drug.getName(), currentCount);
        } else {
            allDrugs.put(drug, (currentCount - count));
            System.out.printf("%s bought %d tablets of %s. Remaining: %d%n", name, count, drug.getName(), (currentCount - count));
        }
    }
