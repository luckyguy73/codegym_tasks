...
public class GameObject {
    ...
    public boolean isCollisionPossible(GameObject otherGameObject) {
        if (x > otherGameObject.x + otherGameObject.width || x + width < otherGameObject.x) return false;
    ...
    private boolean isCollision(int x, int y) {
        for (int matrixX = 0; matrixX < width; matrixX++)
            for (int matrixY = 0; matrixY < height; matrixY++)
                if (matrix[matrixY][matrixX] != 0 && matrixX + this.x == x && matrixY + this.y == y) return true;
        return false;
    }
}